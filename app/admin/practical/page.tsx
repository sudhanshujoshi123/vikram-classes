...
import {
  UploadCloud, FlaskConical, Layers,
  GraduationCap, Book, CheckCircle2,
  X, ArrowLeft, Atom
} from 'lucide-react';

export default function UploadPractical() {
  ...
  const uploadPractical = async () => {
    ...
    try {
      const res = await fetch('/api/admin/practical', { method: 'POST', body: formData });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-4 py-10 relative overflow-hidden">
      ...
      {/* GLOW ORBS */}
      <div style={{ position:'absolute', top:'10%', right:'6%', width:400, height:400, borderRadius:'50%', background:'radial-gradient(circle, rgba(139,92,246,0.1) 0%, transparent 70%)', filter:'blur(50px)', pointerEvents:'none' }} />
      <div style={{ position:'absolute', bottom:'10%', left:'5%', width:350, height:350, borderRadius:'50%', background:'radial-gradient(circle, rgba(99,102,241,0.1) 0%, transparent 70%)', filter:'blur(45px)', pointerEvents:'none' }} />
      ...
      >
        {/* TOP ACCENT */}
        <div style={{ position:'absolute', top:0, left:'20%', right:'20%', height:1, background:'linear-gradient(to right, transparent, rgba(139,92,246,0.6), transparent)', borderRadius:1 }} />
        ...
        <AnimatePresence>
          {success && (
            ...
            >
              ...
              <p style={{ color:'#fff', fontSize:18, fontWeight:800 }}>Practical Uploaded!</p>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ width:48, height:48, borderRadius:14, background:'linear-gradient(135deg, rgba(139,92,246,0.2), rgba(99,102,241,0.2))', border:'1px solid rgba(139,92,246,0.3)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            ...
            <div>
              ...
              <h1 style={{ color:'#fff', fontSize:20, fontWeight:800, margin:0 }}>Upload Practical</h1>
              <p style={{ color:'#6b7280', fontSize:12, marginTop:2 }}>Admin • Practical Management</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <div key={label} style={{ gridColumn: label === 'Subject' ? '1 / -1' : 'auto' }}>
                ...
                <div style={{ position:'relative' }}>
                  ...
                  <select
                    ...
                    style={selectBase}
                    onFocus={e => (e.target as HTMLSelectElement).style.borderColor = 'rgba(139,92,246,0.5)'}
          ...
          <div>
            ...
            <label
              style={{
                ...
                border: file ? '1px solid rgba(16,185,129,0.35)' : '1px dashed rgba(139,92,246,0.3)',
                borderRadius:16, padding:'28px 20px', cursor:'pointer', transition:'all 0.2s',
                background: file ? 'rgba(16,185,129,0.05)' : 'rgba(139,92,246,0.03)',
              }}
              onMouseEnter={e => !file && ((e.currentTarget as HTMLLabelElement).style.background = 'rgba(139,92,246,0.07)')}
              onMouseLeave={e => !file && ((e.currentTarget as HTMLLabelElement).style.background = 'rgba(139,92,246,0.03)')}
            >
              {file ? (
                <>
                ...
                <>
                  <div style={{ width:48, height:48, borderRadius:14, background:'rgba(139,92,246,0.1)', border:'1px solid rgba(139,92,246,0.2)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <UploadCloud size={24} style={{ color:'#a78bfa' }} />
                  </div>
                  <div style={{ textAlign:'center' }}>
          ...
          >
            {loading ? (
              ...
              <><UploadCloud size={16} /> Upload Practical</>
            )}
          </motion.button>
        </div>

        {/* BOTTOM ACCENT */}
        <div style={{ position:'absolute', bottom:0, left:'30%', right:'30%', height:1, background:'linear-gradient(to right, transparent, rgba(139,92,246,0.4), transparent)', borderRadius:1 }} />
      </motion.div>
    </div>
  );
}
